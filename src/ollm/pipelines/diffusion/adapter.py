...
class DiffusionPipelineAdapter(PipelineAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = _DIFFUSION_MODELS[self.model_id]
        overrides = {}
        for field in (
            "download_url",
            "repo_id",
    ...
    def prepare(self, models_dir: str, force_download: bool = False) -> str:
        ...
        if self.config.repo_id:
            ...
            if download_url is None:
                env_key = f"OLLMDIFF_{self.model_id.upper().replace('-', '_')}_URL"
                download_url = os.environ.get(env_key)
            if download_url:
                _maybe_download_zip(download_url, model_dir)
            else:
                raise ValueError(
                    f"Model '{self.model_id}' has no repository or download URL configured. "
                    "Provide download_url=... or set the environment variable "
                    f"OLLMDIFF_{self.model_id.upper().replace('-', '_')}_URL"
                )
